...
if True:
    console = Console(record=True, width=int(400))
else:
...
def validate_thresholds():
    ...
    """
    assert (THRESHOLD_OPERATION_IMBALANCE >= 0.0
            and THRESHOLD_OPERATION_IMBALANCE <= 1.0)
    assert (THRESHOLD_SMALL_REQUESTS >= 0.0
            and THRESHOLD_SMALL_REQUESTS <= 1.0)
    assert (THRESHOLD_MISALIGNED_REQUESTS >= 0.0
            and THRESHOLD_MISALIGNED_REQUESTS <= 1.0)
    assert (THRESHOLD_METADATA >= 0.0
            and THRESHOLD_METADATA <= 1.0)
    assert (THRESHOLD_RANDOM_OPERATIONS >= 0.0
            and THRESHOLD_RANDOM_OPERATIONS <= 1.0)
    ...
    if recommendations:
        if not only_issues:
            ...
            for recommendation in recommendations:
                ...
                if verbose and 'sample' in recommendation:
                    messages.append(
                        Padding(
...
def start(self):
    ...
    log = darshanll.log_open(darshan)
    ...
    filename = check_log_version(darshan, log_version, library_version)
    ...
    if total_size and total_size_stdio / total_size > THRESHOLD_INTERFACE_STDIO:
        ...
        insights_operation.append(
            message(INSIGHTS_STDIO_HIGH_USAGE, TARGET_DEVELOPER, HIGH, issue,
                    recommendation))
    ...
    if 'MPI-IO' not in modules:
        ...
        insights_operation.append(
            message(INSIGHTS_MPI_IO_NO_USAGE, TARGET_DEVELOPER, WARN, issue,
                    recommendation))
    ...
    if 'POSIX' in report.records:
        ...
        if total_writes > total_reads and total_operations and abs(
        ...
        ) / total_operations > THRESHOLD_OPERATION_IMBALANCE:
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_WRITE_COUNT_INTENSIVE, TARGET_DEVELOPER,
                        INFO, issue, None))
        ...
        if total_reads > total_writes and total_operations and abs(
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_READ_COUNT_INTENSIVE, TARGET_DEVELOPER,
                        INFO, issue, None))
        ...
        if total_written_size > total_read_size and abs(
                total_written_size - total_read_size) / (
                    total_written_size +
                    total_read_size) > THRESHOLD_OPERATION_IMBALANCE:
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_WRITE_SIZE_INTENSIVE, TARGET_DEVELOPER,
                        INFO, issue, None))
        ...
        if total_read_size > total_written_size and abs(
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_READ_SIZE_INTENSIVE, TARGET_DEVELOPER,
                        INFO, issue, None))
        ...
        if total_reads_small and total_reads_small / total_reads > THRESHOLD_SMALL_REQUESTS and total_reads_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
            ...
            if 'MPI-IO' in modules:
                recommendation.append({
                    ...
                    Syntax.from_path(os.path.join(
                        ROOT, 'snippets/mpi-io-collective-read.c'),
                                    line_numbers=True,
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_HIGH_SMALL_WRITE_REQUESTS_USAGE,
                        TARGET_DEVELOPER, HIGH, issue, recommendation, detail))
        ...
        if total_writes_small and total_writes_small / total_writes > THRESHOLD_SMALL_REQUESTS and total_writes_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
            ...
            if 'MPI-IO' in modules:
                recommendation.append({
                    ...
                    'sample':
                    Syntax.from_path(os.path.join(
                        ROOT, 'snippets/mpi-io-collective-write.c'),
                                    line_numbers=True,
                                    background_color='default')
                })
            else:
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_HIGH_SMALL_READ_REQUESTS_USAGE,
                        TARGET_DEVELOPER, HIGH, issue, recommendation, detail))
        ...
        if total_operations and total_mem_not_aligned / total_operations > THRESHOLD_MISALIGNED_REQUESTS:
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_HIGH_MISALIGNED_MEMORY_USAGE,
                        TARGET_DEVELOPER, HIGH, issue, None))
        ...
        if total_operations and total_file_not_aligned / total_operations > THRESHOLD_MISALIGNED_REQUESTS:
                ...
                recommendation.append(
                    {
                        ...
                        'sample':
                        Syntax.from_path(os.path.join(
                            ROOT, 'snippets/hdf5-alignment.c'),
                                        line_numbers=True,
                                        background_color='default')
                    }, {
                        'message':
            ...
            if 'LUSTRE' in modules:
                recommendation.append({
                    ...
                    'sample':
                    Syntax.from_path(os.path.join(
                        ROOT, 'snippets/lustre-striping.bash'),
                                    line_numbers=True,
                                    background_color='default')
                })

            insights_metadata.append(
                message(INSIGHTS_POSIX_HIGH_MISALIGNED_FILE_USAGE,
                        TARGET_DEVELOPER, HIGH, issue, recommendation))
        ...
        if max_read_offset > total_read_size:
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_REDUNDANT_READ_USAGE, TARGET_DEVELOPER,
                        WARN, issue, None))
        ...
        if max_write_offset > total_written_size:
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_REDUNDANT_WRITE_USAGE, TARGET_DEVELOPER,
                        WARN, issue, None))
        ...
        if total_reads:
            if read_random and read_random / total_reads > THRESHOLD_RANDOM_OPERATIONS and read_random > THRESHOLD_RANDOM_OPERATIONS_ABSOLUTE:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_RANDOM_READ_USAGE,
                            TARGET_DEVELOPER, HIGH, issue, recommendation))
            else:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_SEQUENTIAL_READ_USAGE,
                            TARGET_DEVELOPER, OK, issue, None))
        ...
        if total_writes:
            if write_random and write_random / total_writes > THRESHOLD_RANDOM_OPERATIONS and write_random > THRESHOLD_RANDOM_OPERATIONS_ABSOLUTE:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_RANDOM_WRITE_USAGE,
                            TARGET_DEVELOPER, HIGH, issue, recommendation))
            else:
                ...
                insights_operation.append(
                    message(INSIGHTS_POSIX_HIGH_SEQUENTIAL_WRITE_USAGE,
                            TARGET_DEVELOPER, OK, issue, None))
            ...
            if total_shared_reads and total_shared_reads_small / total_shared_reads > THRESHOLD_SMALL_REQUESTS and total_shared_reads_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
                ...
                for index, row in shared_files.iterrows():
                    if row['INSIGHTS_POSIX_SMALL_READS'] > (
                            total_shared_reads * THRESHOLD_SMALL_REQUESTS / 2):
                        detail.append({
                            'message':
            ...
            if total_shared_writes and total_shared_writes_small / total_shared_writes > THRESHOLD_SMALL_REQUESTS and total_shared_writes_small > THRESHOLD_SMALL_REQUESTS_ABSOLUTE:
                ...
                for index, row in shared_files.iterrows():
                    if row['INSIGHTS_POSIX_SMALL_WRITES'] > (
                            total_shared_writes * THRESHOLD_SMALL_REQUESTS /
                            2):
                        detail.append({
                            'message':
        ...
        if not has_long_metadata.empty:
                ...
                recommendation.append(
                    {
                        ...
                        'sample':
                        Syntax.from_path(os.path.join(
                            ROOT, 'snippets/hdf5-collective-metadata.c'),
                                        line_numbers=True,
                                        background_color='default')
                    }, {
                        'message':
                        ...
                        'sample':
                        Syntax.from_path(os.path.join(ROOT,
            ...
            insights_metadata.append(
                message(INSIGHTS_POSIX_HIGH_METADATA_TIME, TARGET_DEVELOPER,
        ...
        for index, row in shared_files.iterrows():
            ...
            ) / total_transfer_size > THRESHOLD_STRAGGLERS:
                stragglers_count += 1
        ...
        if stragglers_count:
            ...
            for file in detected_files:
                detail.append({
                    ...
                    format(
                        file[1], file_map[int(file[0])] if full_path else
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_SIZE_IMBALANCE, TARGET_USER, HIGH,
        ...
        for index, row in shared_files_times.iterrows():
            ...
            ) / total_transfer_time > THRESHOLD_STRAGGLERS:
                stragglers_count += 1
        ...
        if stragglers_count:
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_TIME_IMBALANCE, TARGET_USER, HIGH,
                        issue, recommendation, detail))
        ...
        for index, row in aggregated.iterrows():
            ...
            ) / row['POSIX_BYTES_WRITTEN_max'] > THRESHOLD_IMBALANCE:
                imbalance_count += 1
        ...
        if imbalance_count:
            ...
            recommendation = [{
                ...
                'sample':
                Syntax.from_path(os.path.join(
                    ROOT, 'snippets/pnetcdf-hdf5-no-fill.c'),
                                line_numbers=True,
                                background_color='default')
            }, {
                'message':
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_INDIVIDUAL_WRITE_SIZE_IMBALANCE,
                        TARGET_DEVELOPER, HIGH, issue, recommendation, detail))
        ...
        for index, row in aggregated.iterrows():
            ...
            ) / row['POSIX_BYTES_READ_max'] > THRESHOLD_IMBALANCE:
                imbalance_count += 1
        ...
        if imbalance_count:
            ...
            insights_operation.append(
                message(INSIGHTS_POSIX_INDIVIDUAL_READ_SIZE_IMBALANCE,
                        TARGET_DEVELOPER, HIGH, issue, recommendation, detail))
    ...
    if 'MPI-IO' in report.records:
        ...
        if df_mpiio['counters']['MPIIO_COLL_READS'].sum() == 0:
            if total_mpiio_read_operations and total_mpiio_read_operations > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                ...
                for index, row in df_mpiio_collective_reads.iterrows():
                    if (row['MPIIO_INDEP_READS'] + row['MPIIO_INDEP_WRITES']
                        ...
                        ) > THRESHOLD_COLLECTIVE_OPERATIONS and (
                            row['MPIIO_INDEP_READS'] +
                            row['MPIIO_INDEP_WRITES']
                        ) > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                        detail.append({
                            'message':
                ...
                insights_operation.append(
                    message(INSIGHTS_MPI_IO_NO_COLLECTIVE_READ_USAGE,
                            TARGET_DEVELOPER, HIGH, issue, recommendation,
                            detail))
        else:
            ...
            insights_operation.append(
                message(INSIGHTS_MPI_IO_COLLECTIVE_READ_USAGE,
                        TARGET_DEVELOPER, OK, issue))
        ...
        if df_mpiio['counters']['MPIIO_COLL_WRITES'].sum() == 0:
            if total_mpiio_write_operations and total_mpiio_write_operations > THRESHOLD_COLLECTIVE_OPERATIONS_ABSOLUTE:
                ...
                insights_operation.append(
                    message(INSIGHTS_MPI_IO_NO_COLLECTIVE_WRITE_USAGE,
                            TARGET_DEVELOPER, HIGH, issue, recommendation,
                            detail))
        else:
            ...
            insights_operation.append(
                message(INSIGHTS_MPI_IO_COLLECTIVE_WRITE_USAGE,
                        TARGET_DEVELOPER, OK, issue))
        ...
        if df_mpiio['counters']['MPIIO_NB_READS'].sum() == 0:
            ...
            if 'H5F' in modules or has_hdf5_extension:
                recommendation.append({
                    ...
                    'sample':
                    Syntax.from_path(os.path.join(
                        ROOT, 'snippets/hdf5-vol-async-read.c'),
                                    line_numbers=True,
                                    background_color='default')
                })
            ...
            insights_operation.append(
                message(INSIGHTS_MPI_IO_BLOCKING_READ_USAGE, TARGET_DEVELOPER,
        ...
        if df_mpiio['counters']['MPIIO_NB_WRITES'].sum() == 0:
            ...
            if 'H5F' in modules or has_hdf5_extension:
                recommendation.append({
                    ...
                    'sample':
                    Syntax.from_path(os.path.join(
                        ROOT, 'snippets/hdf5-vol-async-write.c'),
                                    line_numbers=True,
                                    background_color='default')
                })
            ...
            insights_operation.append(
                message(INSIGHTS_MPI_IO_BLOCKING_WRITE_USAGE, TARGET_DEVELOPER,
                        WARN, issue, recommendation))
                    ...
                    if 'NNodes' in first:
                        ...
                        if cb_nodes > NUMBER_OF_COMPUTE_NODES:
                            ...
                            recommendation = [{
                                ...
                                'sample':
                                Syntax.from_path(os.path.join(
                                    ROOT, 'snippets/mpi-io-hints.bash'),
                                                line_numbers=True,
                                                background_color='default')
                            }]

                            insights_operation.append(
                                message(INSIGHTS_MPI_IO_AGGREGATORS_INTER,
                                        TARGET_USER, HIGH, issue,
                                        recommendation))
                        ...
                        if cb_nodes < NUMBER_OF_COMPUTE_NODES:
                            ...
                            insights_operation.append(
                                message(INSIGHTS_MPI_IO_AGGREGATORS_INTRA,
                                        TARGET_USER, OK, issue))

                        if cb_nodes == NUMBER_OF_COMPUTE_NODES:
                            issue = 'Application is using one aggregator per compute node'

                            insights_operation.append(
                                message(INSIGHTS_MPI_IO_AGGREGATORS_OK,
                                        TARGET_USER, OK, issue))
    ...
    if json:
        ...
        for key, values in data.items():
            for value in values:
                ...
                insights_dxt.append(
                    message(code, TARGET_DEVELOPER, level, issue,
                            recommendation))
    ...
    if insights_metadata:
        console.print(
            Panel(Padding(Group(*insights_metadata), (1, 1)),
                title='METADATA',
                title_align='left'))

    if insights_operation:
        console.print(
            Panel(Padding(Group(*insights_operation), (1, 1)),
                title='OPERATIONS',
                title_align='left'))

    if insights_dxt:
        console.print(
            Panel(Padding(Group(*insights_dxt), (1, 1)),
                title='DXT',
                title_align='left'))
    ...
    if export_html:
        console.save_html('{}.html'.format(darshan),
                        theme=export_theme,
                        clear=False)

    if export_svg:
        console.save_svg('{}.svg'.format(darshan),
                        title='Drishti',
                        theme=export_theme,
                        clear=False)
    ...
    if export_csv:
        issues = [
            ...
            INSIGHTS_POSIX_HIGH_SMALL_READ_REQUESTS_SHARED_FILE_USAGE,
            INSIGHTS_POSIX_HIGH_SMALL_WRITE_REQUESTS_SHARED_FILE_USAGE,
            INSIGHTS_POSIX_HIGH_METADATA_TIME, INSIGHTS_POSIX_SIZE_IMBALANCE,
            INSIGHTS_POSIX_TIME_IMBALANCE,
            INSIGHTS_POSIX_INDIVIDUAL_WRITE_SIZE_IMBALANCE,
            INSIGHTS_POSIX_INDIVIDUAL_READ_SIZE_IMBALANCE,
            INSIGHTS_MPI_IO_NO_USAGE, INSIGHTS_MPI_IO_NO_COLLECTIVE_READ_USAGE,
            ...
            INSIGHTS_MPI_IO_AGGREGATORS_INTER, INSIGHTS_MPI_IO_AGGREGATORS_OK
        ]
        if codes:
            issues.extend(codes)
        ...
        filename = '{}-summary.csv'.format(darshan.replace(
            '.darshan', ''))
