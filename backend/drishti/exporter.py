...
def message(code, target, level, issue, recommendations=None, details=None):
    ...
    messages = [
        '{}{}{} {}'.format(color, icon, ' [' + code + ']' if args.code else '',
                           issue)
    ]
...
def exporter(arg: Args):
                ...
                if row['total_reads'] > (total_reads *
                    ...
                    detail.append({
                        ...
                        '{} ({:.2f}%) small read requests are to "{}"'.format(
                            ...
                            file_map[int(row['id'])] if args.full_path else
        ...
        if stragglers_count:
            ...
            for file in detected_files:
                detail.append({
                    ...
                    format(
                        file[1], file_map[int(file[0])] if args.full_path else
    ...
    if args.export_html:
        print("export html")
        console.save_html('{}.html'.format(args.darshan),
                          theme=export_theme,
                          clear=False)

    if args.export_svg:
        console.save_svg('{}.svg'.format(args.darshan),
                         title='Drishti',
                         theme=export_theme,
                         clear=False)
    ...
    if args.export_csv:
        ...
        filename = '{}-summary.csv'.format(args.darshan.replace(
            '.darshan', ''))
